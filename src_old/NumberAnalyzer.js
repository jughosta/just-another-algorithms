define(function () {
	...
	NumberAnalyzer.prototype.leastCommonMultiple = function (numberA, numberB) {
		return Math.abs(numberA * numberB /
			this.greatestCommonDivisor(numberA, numberB));
	};
	...
	NumberAnalyzer.prototype.findMaxSubArray = function (array, low, high) {
		...
		if (leftSideResult.maxSum >= rightSideResult.maxSum &&
			leftSideResult.maxSum >= crossSideResult.maxSum) {
			return leftSideResult;
		}

		if (rightSideResult.maxSum >= leftSideResult.maxSum &&
			rightSideResult.maxSum >= crossSideResult.maxSum) {
	 ...
	 * @private
	 */
	NumberAnalyzer.prototype._findMaxCrossingSubArray =
		function (array, low, mid, high) {
	 ...
	 * @return {Number} index
	 */
	NumberAnalyzer.prototype.binarySearch =
		function (array, key, isAlreadySorted) {
	...
	NumberAnalyzer.prototype.find3ElementsThatSumIsZero = function (array) {
		...
		for (i = 0; i < array.length; i++) {
			for (var j = i + 1; j < array.length; j++) {
				k = this.binarySearch(
					sortedArray,
					-(sortedArray[i].key + sortedArray[j].key),
					true
				);
				if (k <= j) {
					continue;
				}
				result.push(sortedArray[i].key + ' ' +
					sortedArray[j].key + ' ' + sortedArray[k].key);
			}
		}
	...
	NumberAnalyzer.prototype.find3ElementsThatSumIsEqual = function (array) {
		...
		for (i = 0; i < array.length; i++) {
			for (var j = i + 1; j < array.length; j++) {
				...
				if (k < 0 || k == j || k == i ||
					result[sortedArray[i].key + ' ' + sortedArray[j].key]) {
					continue;
				}
				result[sortedArray[i].key + ' ' + sortedArray[j].key] =
					sortedArray[i].key + ' ' + sortedArray[j].key + ' ' +
	 ...
	 * @private
	 */
	NumberAnalyzer.prototype._decomposeNumber =
		function (n, component, length, componentsString) {
			if (n === 0) {
			...
			for (var i = 1; i <= n; i++) {
				...
				component[length] = i;
				this._decomposeNumber(n - i, component,
					length + 1, componentsString);
			}
		};
	...
	NumberAnalyzer.prototype.translatePhoneNumberToWord = function (number) {
		...
		var resultString = [],
			results = [];
		this._translateNextPhoneNumber(dictionary, number, resultString,
			0, results);
	 ...
	 * @private
	 */
	NumberAnalyzer.prototype._translateNextPhoneNumber =
		function (dictionary, number, component, index, componentsString) {
			...
			for (var i = 0; i < rowInDictionary.length; i++) {
				...
				this._translateNextPhoneNumber(dictionary, number, component,
					index + 1, componentsString);
			}
		};
	...
	NumberAnalyzer.prototype.sumTwoBinaryNumbers = function (a, b) {
		...
		for (i = 0; i < firstNumber.length; i++) {
			value = (i >= secondNumber.length) ?
				'0' :
				secondNumber[secondNumber.length - i - 1];
			currentSum = this._addBit(
				firstNumber[firstNumber.length - i - 1],
				value, addition
			);
