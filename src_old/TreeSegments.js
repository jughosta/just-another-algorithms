define(function () {
	 ...
	 * @private
	 */
	TreeSegments.prototype._buildSumTree =
		function (tree, a, level, leftIndex, rightIndex) {
			...
			this._buildSumTree(tree, a, level * 2 + 1, middleIndex + 1,
				rightIndex);
	 ...
	 * @private
	 */
	TreeSegments.prototype._getSumInSegment =
		function (tree, level, leftIndex, rightIndex, left, right) {
			if (left > right) {
				return 0;
			}
			...
			var middleIndex = (leftIndex + rightIndex) >> 1;
			return this._getSumInSegment(tree, level * 2, leftIndex, middleIndex,
					left, Math.min(right, middleIndex)) +
				this._getSumInSegment(tree, level * 2 + 1, middleIndex + 1,
					rightIndex, Math.max(left, middleIndex + 1), right);
		};
	 ...
	 * @private
	 */
	TreeSegments.prototype._updateSumTree =
		function (tree, level, leftIndex, rightIndex, position, newValue) {
			if (leftIndex === rightIndex) {
				tree[level] = newValue;
				return;
			}
			...
			if (position <= middleIndex) {
				this._updateSumTree(tree, level * 2, leftIndex, middleIndex,
					position, newValue);
			} else {
				this._updateSumTree(tree, level * 2 + 1, middleIndex + 1,
					rightIndex, position, newValue);
			}
	 ...
	 * @return {Number}
	 */
	TreeSegments.prototype.updateValueInArrayAndGetSum =
		function (position, newValue, left, right) {
			if (!this._tree) {
				return 0;
			}
